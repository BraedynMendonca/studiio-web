...
if (!isSpotifyConfigured()) {
  console.warn(
    "Spotify integration is disabled. Missing NEXT_PUBLIC_SPOTIFY_CLIENT_ID or NEXT_PUBLIC_SPOTIFY_REDIRECT_URI environment variables.",
  )
}
...
export async function redirectToSpotifyAuth() {
  if (!isSpotifyConfigured()) {
    console.error("Spotify is not configured")
    return
  }
  ...
  window.location.href = generateUrlWithSearchParams("https://accounts.spotify.com/authorize", {
    response_type: "code",
    client_id: clientId!,
    scope: scope,
...
export async function getPlaylistTracks(playlistId: string) {
  ...
  while (mostRecentTrackRequestLength >= 100) {
    let uri = `https://api.spotify.com/v1/playlists/${playlistId}/tracks`
